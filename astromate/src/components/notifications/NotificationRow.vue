...
onMounted(()=>{
  text.value = makeText(props.notification.text)
})

function makeText(textNumber: number): string{
  switch (textNumber) {
    ...
    case notificationText.UserWantsUserToHisGroup:
      return "Uživatel " + props.notification.senderName+" vás pozval do skupiny "+props.notification.groupName
    case notificationText.UserWantsToUsersGroup:
      return "Uživatel " + props.notification.senderName+" se chce připojit do skupiny "+props.notification.groupName
    default:
      return ""
  }
}

async function accept(senderId: string, receiverId: string, groupDocumentId: string, userDocumentId: string){
